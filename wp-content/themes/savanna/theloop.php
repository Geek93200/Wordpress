...
<?php // Post featured image
	if(has_post_thumbnail())  {
		if (!is_single()){ ?><a class="featured" href="<?php the_permalink(); ?>" title="<?php printf( $SMTheme->_( 'permalink' ), the_title_attribute( 'echo=0' ) ); ?>"> <?php }
			the_post_thumbnail(
				array(217,217),
				array("class" => $SMTheme->get( 'layout','imgpos' ) . " featured_image")
			);
		if (!is_single()){ ?><div class="hover <?php echo $SMTheme->get( 'layout','imgpos' ); ?>"></div></a><?php }
	}
